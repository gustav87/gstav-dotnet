...
export default class FlickrViewer extends Vue {
  ...
  public async getDogs() {
    try {
      ...
      const response = await fetch(`https://${window.location.hostname}:7248/api/flickr/getDogs`)
  ...
  public async getTaggedImages() {
    try {
      const response = await fetch(`https://${window.location.hostname}:7248/api/flickr/getTaggedImages?tag=${this.tag}`)
      if (response.status === 200) {
        const data = await response.json()
